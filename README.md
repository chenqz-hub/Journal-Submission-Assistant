...
1. 确保已安装 Python，并勾选了 Add Python to PATH。 (Ensure Python is installed and added to PATH)
2. 双击运行 start.bat。 (Double-click start.bat)
3. 首次启动会自动创建虚拟环境并在后台下载依赖，随后提示输入 LLM API Key 和访问口令。 (It will auto-create a virtual environment, install dependencies, and ask for your LLM API Key & Access Code)
4. 服务成功启动后，将自动打开浏览器访问 (Browser will auto-open to): http://127.0.0.1:8000

### 方法 B：公网部署 / 服务器运行 (推荐实验室共享) / Method B: Server Deployment
🇨🇳 工具支持部署到实验室的通用服务器或云服务器上，通过网络共享给课题组小伙伴：
🇬🇧 Deploy on lab or cloud servers to share with your research group:
...
# Edit .env and enter your LLM API_KEY and ACCESS_CODE
...
- 🇨🇳 通过在 .env 中配置 ACCESS_CODE=您的密码 进行保护。
  🇬🇧 Protected by configuring ACCESS_CODE=your_password in .env.
...
├── static/             # HTML/JS/CSS，已注入全局 Auth fetch 拦截器 / Static assets
├── temp/               # 自动化排版缓存输出路径 / Temp cache for document splitting (.gitignore)
├── utils/              # 通用的独立工具函数集合 / Utility functions
├── .env.example        # 环境变量模板 / Env template
├── main.py             # FastAPI App 入口文件 / FastAPI entry point
├── requirements.txt    # 核心依赖清单 / Dependencies
└── start.bat           # Windows 批处理一键启动 / Windows startup batch script
\
---
